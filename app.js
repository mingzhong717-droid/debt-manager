...
// ===== 公共：判断消费是否属于"套现"=====
// 套现特征：category 为"套现"
// 套现记录参与负债/账单计算（银行会出账），但不计入日常消费统计
const CASHOUT_FEE_RATE = 0.006; // 默认手续费率千分之六
function isCashOut(expense) {
  return (expense.category || '') === '套现';
}
function getCashOutFee(amount, rate) {
  return Math.round(amount * (rate || CASHOUT_FEE_RATE) * 100) / 100;
}
...
function getUnpaidTotal(cardId) {
  ...
  // 避免已出账消费与 currentBillAmount 重复计入。
  let acc = null;
  if (DATA && DATA.banks) {
  ...
  if (acc && acc.currentBillAmount > 0) {
    ...
    if (billEndDate) {
      // 有 currentBillEnd：未出账从 currentBillEnd 当天开始（不管是否在未来）
      // 因为 currentBillAmount > 0 表示该周期内的消费已被账单覆盖
      cycleStart = billEndDate.startOf('day');
    } else {
      // 没有 currentBillEnd：按 billDay 推算
...
function renderExpenseStats() {
  ...
  // 分离套现和日常消费
  const cashOutExpenses = expenses.filter(e => isCashOut(e));
  const normalExpenses = expenses.filter(e => !isCashOut(e));
  const total = normalExpenses.reduce((s, e) => s + e.amount, 0);
  const cashOutTotal = cashOutExpenses.reduce((s, e) => s + e.amount, 0);
  ...
  statsEl.innerHTML = `
    ...
    <div class="expense-stat-row">
      <span>笔数</span>
      <span>${normalExpenses.length} 笔</span>
    </div>
    <div class="expense-stat-row">
      <span>日均消费</span>
    ...
    ${cashOutTotal > 0 ? `
    <div class="expense-stat-row" style="margin-top:8px;padding-top:8px;border-top:1px dashed var(--border)">
      <span>💰 套现</span>
      <span>${fmt(cashOutTotal)} (${cashOutExpenses.length}笔)</span>
    </div>
    <div class="expense-stat-row">
      <span style="color:var(--danger)">├ 手续费(千6)</span>
      <span style="color:var(--danger)">${fmt(cashOutFee)}</span>
    </div>` : ''}`;
  ...
  const catColors = {
    ...
    '居家大件': '#a9e34b', '转账还款': '#adb5bd', '套现': '#ffd43b',
    '宠物': '#f783ac', '其他': '#868e96'
  };
...
function renderExpenseOverview() {
  ...
  Object.entries(CARD_BILLING).forEach(([cardId, cfg]) => {
    ...
    if (!cycle) return;
    let { cycleStart, cycleEnd } = cycle;
    // 与 getUnpaidTotal 一致：如果 currentBillAmount > 0，调整 cycleStart 避免重复计入
    let accData = null;
    if (DATA && DATA.banks) {
    ...
    if (accData && accData.currentBillAmount > 0) {
      ...
      if (billEndDate) {
        ...
        const billDay = cfg.billDay;
        if (today.date() >= billDay) {
          cycleStart = today.date(billDay).startOf('day');
        } else {
          cycleStart = cycleEnd.add(1, 'day');
        }
      }
...
function buildDebtContext() {
  ...
  const expenses = (typeof getExpenses === 'function' ? getExpenses() : (DATA.expenses || []));

  // 账户精简版（只保留 AI 需要的字段，去掉冗余）
  const accounts = [];
  (DATA.banks || []).forEach(bank => {
    (bank.accounts || []).forEach(acc => {
      if (acc.type === 'savings') return;
      const base = { bank: bank.name, name: acc.name, type: acc.type, dueDay: acc.dueDay, note: acc.note || '' };
      if (acc.type === 'loan') {
        accounts.push({ ...base, totalDebt: acc.totalDebt, monthlyPayment: acc.monthlyPayment, interestRate: acc.interestRate, remainingMonths: acc.remainingMonths, endDate: acc.endDate });
      } else if (acc.type === 'credit') {
        accounts.push({ ...base, id: acc.id, currentBillAmount: acc.currentBillAmount, paidAmount: acc.paidAmount, minPayment: acc.minPayment, interestRate: acc.interestRate,
          installments: (acc.installments || []).map(i => ({ name: i.name, remainingAmount: i.remainingAmount, monthlyPayment: i.monthlyPayment, remainingMonths: i.remainingMonths, endDate: i.endDate }))
        });
      } else {
        accounts.push({ ...base, minPayment: acc.minPayment });
      }
    });
  });
  ...
  const ctx = {
    ...
    totalDebt, monthlyDue, baseIncome,
    monthlySurplus: baseIncome > 0 ? +(baseIncome - monthlyDue).toFixed(2) : null,
    wallets: wallets.map(w => ({ name: w.name, balance: w.balance })),
    walletTotal: +walletTotal.toFixed(2),
    accounts,
    recentExpenses: expenses.slice(-15).reverse().map(e => ({ date: e.date, amount: e.amount, category: e.category, payment: e.payment, note: e.note || '' })),
  };

  return `【我的负债数据快照 · ${ctx.date}】
...
category 只能是：餐饮堂食、外卖、买菜生鲜、烟酒零食、交通出行、购物数码、购物服装、日用百货、娱乐休闲、订阅会员、医疗健康、教育学习、居家大件、转账还款、套现、宠物、其他
payment 只能是：招商信用卡、广州银行信用卡、浦发信用卡、农行信用卡、民生信用卡、花呗、美团月付、微信零钱、支付宝余额、储蓄卡、现金
用户说"微信""微信支付""微信零钱"均填"微信零钱"，说"支付宝"均填"支付宝余额"
当用户说"套现""TX""刷卡取现""刷出来"等，category填"套现"。套现金额记全额（和银行账单对应），手续费由系统自动计算展示，不需要单独录入手续费。
  ...
  el.innerHTML = aiChatBubbles.map((b, idx) => {
    if (b.role === 'user') {
      ...
      if (!b.parsed) {
        ...
        // 兼容旧历史气泡：b.text 可能是原始 JSON 字符串（如 {"intent":"query","reply":"..."}）
        let displayText = b.text;
        if (!b.streaming && displayText && displayText.trim().startsWith('{')) {
          try {
            const oldParsed = JSON.parse(displayText.trim());
            if (oldParsed.intent === 'query' || oldParsed.intent === 'chat') {
              displayText = oldParsed.reply || displayText;
            }
          } catch {}
        }
        return `<div class="ai-bubble ai-bubble-ai ai-bubble-markdown"><div class="ai-md-body">${renderMarkdown(displayText)}</div>${streamingDots}</div>`;
      }
      const p = b.parsed;
