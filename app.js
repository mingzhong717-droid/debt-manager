...
function buildDebtContext() {
  ...
  const summary = {
    ...
    monthlyDue,
    baseIncome,
    monthlySurplus: baseIncome > 0 ? baseIncome - monthlyDue : null,
    walletTotal,
    expenseCount: expenses.length,
    recentExpenses: expenses.slice(-20).reverse(),  // 最近20条消费
  };

  return `【我的完整负债数据 · ${DATA.meta.lastUpdated}】

// 计算汇总（已算好，直接用）
${JSON.stringify(summary, null, 2)}

// 原始数据（所有字段，包含账户/分期/钱包/收入等完整信息）
